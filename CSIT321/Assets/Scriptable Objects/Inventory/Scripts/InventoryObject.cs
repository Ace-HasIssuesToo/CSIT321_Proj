using System.Collections;
using System.Collections.Generic;
using UnityEngine;

[CreateAssetMenu(fileName = "New Inventory", menuName = "Inventory System/Inventory")]
public class InventoryObject : ScriptableObject
{
    //List of InventorySlot to hold our items
    public List<InventorySlot> Container = new List<InventorySlot>();

    public void AddItem(ItemObject _item, int _amount)
    {
        //find out whether the item we are trying to add in our inventory or not
        bool hasItem = false;
        for (int i = 0; i < Container.Count; i++)
        {
            if (Container[i].item == _item)
            {
                //loop true our Container(inventory), if item matching, set hasItem to true and add to that amount (Stacking)
                Container[i].AddAmount(_amount);
                hasItem = true;
                break;
            }
        }
        if (!hasItem)   //if we do not have the item in inventory...
        {
            Container.Add(new InventorySlot(_item, _amount));
        }
    }
}

[System.Serializable]       //when adding this class to an object, shows up in editor
public class InventorySlot
{
    public ItemObject item;
    public int amount;

    //constructor
    public InventorySlot(ItemObject _item, int _amount)
    {
        item = _item;
        amount = _amount;
    }

    //add amount of object to InventorySlot
    public void AddAmount(int value)
    {
        amount += value;
    }
}
